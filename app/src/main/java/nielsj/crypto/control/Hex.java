...
public class Hex {
  ...
  // conversion of string of hex digits
  // to byte array
  public static byte[] hexStringToByteArray(String s) {
    String us = s.toUpperCase();
    int characters = us.length();
    if ( isOdd(characters) ) {
      System.out.println("Util-exception: uneven size of hex string ??");
      return new byte[0];
    }
  ...
  private static String byteToHexPair(int b) {
    // rename to hexpair
